...
import { setDescription, setName, setPrice, setProject, setTeam } from "./licenseSlice";
...
export default function CreateLicenseForm(props: CreateLicenseFormProps) {
  ...
  const [_price, _setPrice] = useState<string>('0');
...
`);
  ...
  useEffect(() => {
    ...
    (async () => {
      let isNameTaken = _name?.length > 0 && await checkLicenseName(_name);
  ...
  useEffect(() => {
    if (_name && validName && _price) {
      setValidForm(true);
    } else {
      setValidForm(false);
    }
  }, [_name, _price, validName]);
      ...
      <div>
        ...
        <div className="mt-1">
          <input
            ...
            type="number"
            min="0"
            onChange={(e) => _setPrice(e.target.value.toString())}
            value={_price}
            required={true}
