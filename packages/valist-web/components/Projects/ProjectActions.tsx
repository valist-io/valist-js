...
const ProjectActions = (props: ProjectActionsProps) => {
  ...
  const loadAction = () => {
    try {
      return projectTypes['binary'][actionView];
    } catch (err) {
      return undefined;
    }
  };
  ...
  useEffect(() => {
    ...
    setActions(GetActions(origin, teamName, projectName));
  }, []);

  return (
    <div className="pb-4">
      {(loadAction() && Object.keys(actions).length
        && projectTypes['binary'][actionView].map((action: string) => (
