...
export default class Client {
  ...
  async createAccount(
    ...
    const unsigned = await this.registry.populateTransaction.createAccount(
      name,
      metaURI,
      members
    );
  ...
  async createProject(
    ...
    const metaURI = await this.writeJSON(meta);
    const unsigned = await this.registry.populateTransaction.createProject(
      accountID,
      name,
      metaURI,
      members
  ...
  async uploadRelease(config: ReleaseConfig): Promise<ReleaseMeta> {
    ...
    for (let i = 0; i < platforms.length; i++) {
      if (config.platforms[platforms[i] as SupportedPlatform]) {
        filesObject[platforms[i]] = await getFilesFromPath(
          config.platforms[platforms[i] as SupportedPlatform]
        );
      }
    }
    ...
    const nonWebIC: ImportCandidate[] = Object.entries(nonWebFiles).flatMap(
      ([platform, files]) =>
        files.map((file) => ({
  ...
  async createRelease(
    ...
    const metaURI = await this.writeJSON(meta);
    const unsigned = await this.registry.populateTransaction.createRelease(
      projectID,
      name,
      metaURI
    );
  ...
  async setAccountMeta(
    ...
    const metaURI = await this.writeJSON(meta);
    const unsigned = await this.registry.populateTransaction.setAccountMetaURI(
      accountID,
      metaURI
    );
    return await this.sendTx(unsigned);
  ...
  async setProjectMeta(
    ...
    const metaURI = await this.writeJSON(meta);
    const unsigned = await this.registry.populateTransaction.setProjectMetaURI(
      projectID,
      metaURI
    );
    return await this.sendTx(unsigned);
  ...
  async addAccountMember(
    ...
    const unsigned = await this.registry.populateTransaction.addAccountMember(
      accountID,
      address
    );
    return await this.sendTx(unsigned);
  ...
  async removeAccountMember(
    ...
    const unsigned =
      await this.registry.populateTransaction.removeAccountMember(
        accountID,
        address
      );
    return await this.sendTx(unsigned);
  ...
  async addProjectMember(
    ...
    const unsigned = await this.registry.populateTransaction.addProjectMember(
      projectID,
      address
    );
    return await this.sendTx(unsigned);
  ...
  async removeProjectMember(
    ...
    const unsigned =
      await this.registry.populateTransaction.removeProjectMember(
        projectID,
        address
      );
    return await this.sendTx(unsigned);
  ...
  async approveRelease(
    ...
    const unsigned = await this.registry.populateTransaction.approveRelease(
      releaseID
    );
    return await this.sendTx(unsigned);
  }
  ...
  async withdrawProductTokenBalance(
    token: string,
    projectID: ethers.BigNumberish,
    recipient: string
  ): Promise<ContractTransaction> {
  ...
  async getProductBalanceBatch(
    addresses: string[],
    projectIDs: ethers.BigNumberish[]
  ): Promise<ethers.BigNumber[]> {
  ...
  async writeFile(
    ...
    onProgress?: (bytes: string) => void
  ) {
    if (typeof file === "undefined")
      throw new Error("file === undefined, must pin at least one file");

    let fileData, fileSize: number;
    const baseFileName = this.getFileBaseName(file.name);
    const path = wrapWithDirectory
    ...
    const res = await this.ipfs.add(fileData, {
      ...
      progress: (bytes: number) =>
        onProgress ? onProgress(formatBytes(String(bytes))) : "",
    });
  ...
  async writeFolder(
    ...
    if (files.length == 0)
      throw new Error("files.length == 0, must pin at least one file");
    ...
    for await (const res of this.ipfs.addAll(files, {
      ...
      progress: (bytes: number, path?: string) =>
        onProgress
          ? onProgress(
              `${path ? `(${path})  ` : ""}${formatBytes(String(bytes))}`
            )
          : "",
    })) {
